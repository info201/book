...
However, this NSE can sometimes trip you up when using more complex functions such as `arrange()` or `group_by()`, or when you want to create your own functions that use NSE.

```r
which.col <- 'mpg'
arrange(mtcars, which.col)  # Error in arrange_impl(.data, dots) :
                            #   incorrect size (1) at position 1, expecting : 32
```

In this case, the `arrange()` function is trying to "quote" what we typed in (the `which.col` variable name&dmash;not it's `mpg` value), and then hitting a problem because there is no column of that name (it can't resolve that column name to a column index).
...
Second, you will need to tell the `arrange()` function that it should _not_ quote this symbol (because you've already converted it into a variable)&mdash;what is called **unquoting**. In `dplyr`, you "unquote" a parameter to a method by including two exclamation points in front of it:

```r
arrange(mtcars, !!which.col.sym)  # arranges by the specified column
```
