...
The `filter()` function takes in the data frame to filter, followed by a comma-separated list of conditions that each _row_ returned row must satisfy. Note again that columns are provided without quotation marks!
...
storms <- mutate(storms, ratio = pressure/wind)  # Replace existing `stroms` frame with mutated one!
```
