...
If you are sure you want to add _all the contents_ of the directory (tracked or untracked) to the staging area, you can do it with:

```sh
git add .
```

<p class="alert alert-danger">**WARNING**: This will add _everything_ in
...
instead of `git add` and `git commit` separately.  the `-a` option tells git to include _all modified files_ into this commit.
