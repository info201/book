...
```bash 
scp
Desktop/info201/cool_script.R myusername@server.ischool.edu:scripts/
``` 
(here we assume the current directory of your laptop is the one above
`Desktop`.) 
...
frontend for copying files between a local and a remote machine over scp
...
here is no RStudio to handrail you through the session.  I strongly
recommend to change to the directory where you intend to run the
project before starting R (`cd scripts`).  Now all loading,
...
resources as you wish, this is not true for the server.  The server is a
multiuser system, potentially doing good work concurrently for many people.  So
the first rule is:
...
This most likely means you grabbed too much memory, and the system
just killed your script.  Deal with it.
...
cannot kill processes you did not start.  Feel free and attempt.  It
does not work.
