...
Thus you can equivalently talk about accessing a particular **resource** and sending a request to a particular **endpoint**. The **endpoint** is appended to the end of the **Base URI**, so you could access Spotify's new releases by combining the **Base URI** (`https://api.spotify.com`) and **endpoint** (`/v1/browse/new-releases`) into a single string: `https://api.spotify.com/v1/browse/new-releases`. That URL will return a data structure of new releases, which you can request from your R program or simply view in your web-browser.
